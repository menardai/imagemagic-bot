...
class SlackInput(InputChannel):
    ...
    def _is_user_message(slack_event):
        return (
            ...
            and (
                slack_event.get("event").get("text")
                or slack_event.get("event").get("subtype") == "file_share"
            )
    ...
    async def process_message(self, request: Request, on_new_message, text, sender_id, files=None):
